/**
 * Names(s) and ID's: Diana Edvi (40198139) AND Fuad Awad (40195634)
 * COMP249
 * Assignment # 2
 * Due Date: November 11, 2023
 */

// -----------------------------------------------------
// Assignment 2
// Question: (Part 1)
// Written by: Diana Edvi (40198139) AND Fuad Awad (40195634)
// -----------------------------------------------------
/**
 * The TooManyFieldsException class represents an exception that is thrown when there are too many fields or elements
 * than required in a book entry.
 */
public class TooManyFieldsException extends Exception{
    /**
     * Constructs a new default TooManyFieldsException.
     */
    public TooManyFieldsException() {
        super();
    }
    /**
     * Constructs a new parameterized TooManyFieldsException with a custom message.
     *
     * @param message a description of the exception
     */
    public TooManyFieldsException(String message) {
        super(message);
    }

}
